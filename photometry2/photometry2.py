...
hdu2 = fits.open("NGC436_b15_wcs.fits")
wcs_head = hdu2[0].header
wcs=WCS(wcs_head)

ra, dec, V, B = np.loadtxt("NGC436_b15.csv", unpack=True, skiprows=1, delimiter=',')
x_pix, y_pix = wcs.all_world2pix(ra*u.deg, dec*u.deg, 1)

#get inst b
hdu = fits.open("NGC436_b15_comb.fit")
data = hdu[0].data
...
#get inst v
hdu = fits.open("NGC436_v5_comb.fit")
data = hdu[0].data
apers = [star, bkgd]
phot_table = aperture_photometry(data, apers)
bkgd_flux = phot_table['aperture_sum_1'] / bkgd.area * star.area
...
plt.scatter(corrected_BV, corrected_V)
